      ...
      {showingRejected && (
        <div className="space-y-4 mt-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <Button onClick={() => setShowingRejected(false)} className="bg-[#2B2521] text-white px-6 py-2 rounded hover:bg-gray-800">Back</Button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rejectedLeads.map((lead, idx) => (
              <div key={idx} className="bg-white rounded-[12px] p-6 shadow border border-red-200 flex flex-col justify-between min-h-[340px]">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Briefcase className="h-5 w-5 text-[#97A88C]" />
                      <span className="font-bold text-xl text-[#97A88C]">{lead.company_name || 'No Company Name'}</span>
                    </div>
